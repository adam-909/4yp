...
class ModelFeatures:
    ...
    def __init__(
        ...
        df,
        total_time_steps,
        start_boundary=1990, # TODO edit?
        test_boundary=2020, # TODO edit?
        test_end=2023,
        ...
        self._column_definition = [
            ...
            ("macd_2_8", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("macd_4_16", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("macd_8_32", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            
            # New Features
            ("log_moneyness", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("time_to_expiry", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
    ...
    def _batch_data(self, data, sliding_window):
        ...
        if sliding_window:
            ...
            for _, sliced in data.groupby(id_col):
                ...
                sequence_lengths = np.array(sequence_lengths, dtype=np.int32) ## TODO old: np.int
            ...
            # Combine all data
            for k in data_map:
                print("len(data_map[k]):", len(data_map[k])) # length: 
                data_map[k] = np.concatenate(data_map[k], axis=0)
